use parquet2::{
    deserialize::{BinaryPage, BinaryPageValues},
    error::Result,
    page::DataPage,
};
...
pub fn page_to_vec(page: &DataPage, dict: Option<&BinaryPageDict>) -> Result<Vec<Option<Vec<u8>>>> {
    ...
    match state {
        BinaryPage::Optional(validity, values) => match values {
        ...
        BinaryPage::Required(values) => match values {
        ...
        BinaryPage::Levels(validity, max, values) => {
